...
public class NettyClient {
    ...
    public void start() {
        try {
            ChannelFuture future = boot.connect(node.getIp(), node.getPort()).addListener(new ChannelFutureListener() {
                @Override
                public void operationComplete(ChannelFuture future) throws Exception {
                    if (future.isSuccess()) {
                        ...
                        NetworkTest.removeNode(node.getId());
                    }
                }
            });
            ...
            Log.info("Client start exception:" + e.getMessage() + ", remove node: " + node.getId());
            NetworkTest.removeNode(node.getId());
        }
    }
